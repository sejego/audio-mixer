...
int udp_client_setup(char *broadcast_address, int broadcast_port) {
	...
	/* Enable broadcast */
	if (setsockopt(client_socket, SOL_SOCKET, SO_BROADCAST, (void *) &broadcast_enabled,
 ...
 *
 * @param:
 * int16_t *buffer: Pointer to the data to the buffer where the received data gets stored
...
int udp_client_recv(int16_t *buffer,int buffer_size ) {
    ...
    if((recvfrom(client_socket, buffer, buffer_size, 0,
        (struct sockaddr *)&receiving_address, &addr_size)) != -1 ) {
